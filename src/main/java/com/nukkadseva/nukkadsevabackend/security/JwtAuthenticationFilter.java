        ...
        if (jwt != null && SecurityContextHolder.getContext().getAuthentication() == null) {
            try {
                ...
                if (email != null && jwtUtil.validateToken(jwt, email)) {
                    // Extract role from JWT and convert to Spring Security authority (add ROLE_ prefix)
                    String role = jwtUtil.extractRole(jwt);
                    Long userId = jwtUtil.extractUserId(jwt);
                    Long profileId = jwtUtil.extractProfileId(jwt);
                    AuthUser authUser = new AuthUser(userId, email, role, profileId);
